...
const effects: Effect[] = [
  ...
  },
  {
    ...
    onPlayed: async (state) => await state.draw(1 + advantage),
  ...
  },
  {
    ...
    onPlayed: async (state) => await state.draw(2 + advantage),
  ...
  },
  {
    description: `La prochaine carte jouée coûte la moitié de son prix${
      advantage > 3
        ? ` et gagne ${advantage - 3} @energy${advantage - 3 > 1 ? "s" : ""}`
        : ""
  ...
  },
  {
    ...
    onPlayed: async (state) => await state.addEnergy(4 + advantage),
    type: "action",
